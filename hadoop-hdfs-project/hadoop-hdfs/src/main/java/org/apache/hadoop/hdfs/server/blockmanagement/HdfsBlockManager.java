...
public class HdfsBlockManager implements BlockManager {
  ...
  public boolean shouldUpdateBlockKey(final long updateTime)
      throws IOException {
    return isBlockTokenEnabled() &&
        blockTokenSecretManager.updateKeys(updateTime);
  }
  ...
  public Object setProperty(String property, String newVal)
      throws IllegalArgumentException{
    int propertyValue;
    switch (property) {
    case DFSConfigKeys.DFS_NAMENODE_REPLICATION_MAX_STREAMS_KEY:
      propertyValue =
          adjustNewVal(DFS_NAMENODE_REPLICATION_MAX_STREAMS_DEFAULT, newVal);
      ensurePositiveInt(propertyValue,
          DFSConfigKeys.DFS_NAMENODE_REPLICATION_MAX_STREAMS_KEY);
      maxReplicationStreams = propertyValue;
      break;
    case DFSConfigKeys.DFS_NAMENODE_REPLICATION_STREAMS_HARD_LIMIT_KEY:
      propertyValue = adjustNewVal(
          DFS_NAMENODE_REPLICATION_STREAMS_HARD_LIMIT_DEFAULT, newVal);
      ensurePositiveInt(propertyValue,
          DFSConfigKeys.DFS_NAMENODE_REPLICATION_STREAMS_HARD_LIMIT_KEY);
      replicationStreamsHardLimit = propertyValue;
      break;
    case DFSConfigKeys.DFS_NAMENODE_REPLICATION_WORK_MULTIPLIER_PER_ITERATION:
      propertyValue =  adjustNewVal(
          DFS_NAMENODE_REPLICATION_WORK_MULTIPLIER_PER_ITERATION_DEFAULT,
          newVal);
      ensurePositiveInt(propertyValue,
          DFSConfigKeys.DFS_NAMENODE_REPLICATION_WORK_MULTIPLIER_PER_ITERATION);
      blocksReplWorkMultiplier = propertyValue;
      break;
    default:
  ...
  private LocatedBlock createLocatedBlock(LocatedBlockBuilder locatedBlocks,
    ...
    if (!blk.isComplete()) {
      ...
      if (blk.isStriped()) {
        ...
        final ExtendedBlock eb = new ExtendedBlock(getBlockPoolId(),
            blk);
        return BlockManager.newLocatedStripedBlock(eb, storages,
            uc.getBlockIndices(), pos, false);
      } else {
        final DatanodeStorageInfo[] storages = uc.getExpectedStorageLocations();
        final ExtendedBlock eb = new ExtendedBlock(getBlockPoolId(),
            blk);
        return null == locatedBlocks
            ? BlockManager.newLocatedBlock(eb, storages, pos, false)
    ...
    return blockIndices == null
        ? null == locatedBlocks ?
        BlockManager.newLocatedBlock(eb, machines, pos, isCorrupt)
        : locatedBlocks.newLocatedBlock(eb, machines, pos, isCorrupt)
        : BlockManager.newLocatedStripedBlock(
            eb, machines, blockIndices, pos, isCorrupt);
  }
  ...
  public void removeBlocksAssociatedTo(final DatanodeDescriptor node) {
  ...
  public void removeBlocksAssociatedTo(final DatanodeStorageInfo storageInfo) {
   ...
   * {@link BlockManager#processMisReplicatedBlocks()}.
   */
   ...
   * @throws IOException 
   */
  public void processFirstBlockReport(
      final DatanodeStorageInfo storageInfo,
